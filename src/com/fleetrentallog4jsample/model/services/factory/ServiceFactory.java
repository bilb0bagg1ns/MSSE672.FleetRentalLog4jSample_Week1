 ...
 * fleetRentalController.performAction("ProcessItineraryService",
 * rentalComposite);
 *
 * 2. Controller then calls the FleetRentalManager to perform an action. Here
 * commandString = "ProcessItineraryService" from step 1
 ...
 *
 * 4. Important to note here that all Service classes
 * (com.fleetrental.business.services.*) implement IService interface.
 *
 * Hence, the getService method when done, instead of returning
 * ProcessItineraryService class returns a IService interface class which
 * continues the decoupling theme of MVC so the calling classes don't deal with
 * a concrete ProcessItineraryService class but its interface.
 *
